...
export class CustomerController {
  ...
  async verifyRegister(@Body() payload: VerifyRegisterDto) {
    const results = await this.customerService.verifyRegister({ payload, active: null });
  ...
  async findAll(@Query() queries: AQueries<CustomerEntity>, @ActiveUser() active: TPayloadToken) {
    const { items, totalItems } = await this.customerService.findAll({ queries, active });
