...
export class KeywordService {
  ...
  async create({ payload, active }: ICreateService<CreateKeyWordDto>): Promise<KeywordEntity> {
    //
  ...
  async update({ id, payload, active }: IUpdateService<UpdateKeyWordDto>): Promise<KeywordEntity> {
    //
    const key = createKeyUserActive(active.userId);
