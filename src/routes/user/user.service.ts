...
export class UserService implements OnModuleInit {
  ...
  async create({ payload, active }: ICreateService<CreateUserDto>): Promise<UserEntity> {
  ...
  async findAll({ queries, active }: IFindAllService<UserEntity>): Promise<IGetMulti<UserEntity>> {
    //
    const key = createKeyUserActive(active?.userId);
  ...
  async update({ id, payload, active }: IUpdateService<UpdateUserDto>): Promise<UserEntity> {
