...
export class RoleGroupService {
  ...
  async create({ payload, active }: ICreateService<CreateRoleGroupDto>): Promise<RoleGroupEntity> {
  ...
  async findAll({ queries, active }: IFindAllService<IRoleGroup>): Promise<IGetMulti<RoleGroupEntity>> {
    //
    const key = createKeyUserActive(active.userId);
  ...
  async update({ id, payload, active }: IUpdateService<UpdateRoleGroupDto>): Promise<RoleGroupEntity> {
    const { roles } = payload;
