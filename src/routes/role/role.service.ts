...
export class RoleService {
  ...
  async create({ payload, active }: ICreateService<CreateRoleDto>): Promise<RoleEntity> {
    //
  ...
  async findAll({ queries, active }: IFindAllService<IRole>): Promise<IGetMulti<RoleEntity>> {
    //
    const key = createKeyUserActive(active.userId);
  ...
  async update({ id, payload, active }: IUpdateService<UpdateRoleDto>): Promise<RoleEntity> {
    //
    const key = createKeyUserActive(active.userId);
